...
```bash
COSC473$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
COSC473$ sam logs -n HelloWorldFunction --stack-name COSC473 --tail
```
...
```bash
COSC473$ pip install pytest pytest-mock --user
COSC473$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name COSC473
```
